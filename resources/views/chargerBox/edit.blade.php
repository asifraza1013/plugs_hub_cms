...
@section('content')
    @include('users.partials.header', ['title' => __('Edit Charger Info')])
        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('chargerbox.update', $charger) }}" autocomplete="off" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old('name', $charger->name) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-control-alternative{{ $errors->has('type') ? ' is-invalid' : '' }}" name="type">
                                        ...
                                        @foreach (config('constants.charger_info') as $key=>$item)
                                            <option value="{{ $key }}"
                                            {{ ($charger->type == $key) ? 'select' : '' }}
                                            >{{ $item }}</option>
                                        @endforeach
                                        <option value="1" {{ ($charger->type == 1) ? 'selected' :'' }}>{{ __('Charger Box') }}</option>
                                        <option value="2" {{ ($charger->type == 2) ? 'selected' :'' }}>{{ __('Charger Plug Type') }}</option>
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <select class="form-control form-control-alternative{{ $errors->has('status') ? ' is-invalid' : '' }}" name="status">
                                        <option disabled selected value> -- {{ __('Select an option') }} -- </option>
                                        <option value="Active" {{ ($charger->status == 'Active') ? 'selected' :'' }}>{{ __('Active') }}</option>
                                        <option value="Inactive" {{ ($charger->status == 'Inactive') ? 'selected' :'' }}>{{ __('Inactive') }}</option>
                                    </select>
