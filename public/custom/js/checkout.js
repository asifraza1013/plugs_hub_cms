...
// check when user click checkout
$('#stripe-checout').on('click', function(){
    console.log('clicked for chekcout');
    ...
    if(cardNumber.length != 19 || cardCvc.length != 3 || cardMonth.length != 2 || cardYear.length != 2){
        alert('Please inset valid card details. As It is required to make order');
        return false;
    }

    generateStripeToken(cardNumber, cardCvc, cardMonth, cardYear);

})

/**
 * generate token for stripe. 
 */
function generateStripeToken(cardNumber, cardCvc, cardMonth, cardYear){
    console.log('here in generate func');

    // stripe publish key. 
    // var publishKey = "{{ env('STRIPE_KEY') }}";
    var publishKey = "pk_test_51J6tRgISGhQT2I3Sn7GzjBOYJeJOklMMAZpyWiNWs0XhFTGS89wjChJigqaszpfXGEIiVdbShG3NraDflkxcnNi800sqeMkFX2";
